...
import { ArrowRight, Loader2, AlertTriangle, ChevronRight, FileSearch } from "lucide-react";
  ...
  return (
    <>
      ...
      {status === "success" && result && (
        <section className="bg-(--color-surface) px-6 py-10 lg:px-10 lg:py-12">
          <motion.div
            initial={{ opacity: 0, y: 14 }}
            animate={{ opacity: 1, y: 0 }}
            transition={{ duration: 0.5, ease: easeOut }}
            className="mx-auto flex max-w-4xl flex-col items-center gap-4 rounded-[20px] border border-(--color-line) bg-white p-6 text-center shadow-[0_20px_50px_-30px_rgba(11,28,77,0.3)] sm:flex-row sm:justify-between sm:text-left"
          >
            <div className="flex w-full min-w-0 items-center gap-4 sm:w-auto">
              <span
                className={`flex h-14 w-14 shrink-0 items-center justify-center rounded-full text-[19px] font-extrabold ${
                  result.score >= 80
              ...
              <div className="min-w-0 flex-1 text-left">
                <p className="truncate text-[15.5px] font-bold text-(--color-ink)">
                  {result.siteTitle || result.finalUrl}
                </p>
                <p className="text-[13px] text-(--color-ink-soft)">
                  Scored {result.scoreLabel} — full report ready
                </p>
              </div>
            </div>
            <button
              ...
              className="group inline-flex shrink-0 items-center gap-2 rounded-full bg-(--color-blue) px-6 py-3 text-[14.5px] font-semibold text-white transition-colors hover:bg-(--color-ink)"
            >
              <FileSearch size={17} />
              View Full Report
      ...
      {result && (
        <ResultsModal
          ...
          title="Shopify Speed & App Bloat Report"
        >
