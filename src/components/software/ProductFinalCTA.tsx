...
export default function ProductFinalCTA({ slug }: ProductFinalCTAProps) {
  const product = getSoftwareProduct(slug);
  if (!product) return null;
  const { name: productName, closingHeadline: headline, closingSubtext: subtext, accentColor } = product;
  ...
  return (
    <section className="relative overflow-hidden bg-(--color-navy-deep) py-20 lg:py-24">
      ...
      >
        <rect x="420" y="180" width="380" height="18" rx="9" transform="rotate(-28 420 180)" fill={accentColor} />
      ...
      <div className="relative mx-auto max-w-4xl px-6 text-center lg:px-10">
        ...
        <div className="mt-9 flex flex-wrap items-center justify-center gap-4">
          <Link
            href="/contact"
            className="group inline-flex items-center gap-2 rounded-full px-8 py-4 text-[15px] font-semibold text-white transition-colors hover:bg-white hover:text-(--color-ink)"
